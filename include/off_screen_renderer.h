...
namespace OffScreenRenderer {
	...
	unsigned char getMaxDepth (unsigned char *Pixels) {
		unsigned char max_depth = 0;
		for (int i = 0; i < WIDTH * HEIGHT; i++) {
			if (Pixels[i] == 255) {
				continue;
			} else if (Pixels[i] > max_depth) {
				max_depth = Pixels[i];
			}
		}
		return max_depth;
	...
	unsigned char *getPixels (void **depthBuffer) {
		const GLubyte *ptr = *(GLubyte**) depthBuffer;

		unsigned char *Pixels = (unsigned char*) malloc(WIDTH * HEIGHT * sizeof(unsigned short int));

		for (int i = 0; i < WIDTH * HEIGHT; i++) {
			unsigned short int val = ptr[2*i] | (ptr[2*i+1] << 8);
			Pixels[i] = val / 256;
		}

		unsigned char max_depth = getMaxDepth(Pixels);

		// Normalize
		for (int i = 0; i < WIDTH * HEIGHT; i++) {
			if (Pixels[i] < 255) {
				Pixels[i] *= 128.0 / max_depth;
			} else {
				Pixels[i] = 255;
			}
		}
	...
	unsigned char *getPixels2 (float *depth_z) {
	...
	inline unsigned char *render(Mesh mesh, Vector cam, int total = -1, bool inverse = false, double maxD = -1) {
		...
		OSMesaGetDepthBuffer(ctx, &frameWidth, &frameHeight, &z, depthBuffer);

		// unsigned char *Pixels = getPixels(depthBuffer);

		// Method 2: Using glReadPixels
		...
		unsigned char *Pixels = getPixels2(depth_z);
